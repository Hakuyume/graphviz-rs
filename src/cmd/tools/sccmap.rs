...
unsafe extern "C" fn visit(
    ...
    *fresh6 = (*fresh6).wrapping_add(1);
    ...
    e = agfstout((*n).root, n);
    while !e.is_null() {
        t = (*if ((*(e as *mut Agobj_t)).tag).objtype() as libc::c_int == 2 as libc::c_int {
            e
        } else {
            e.offset(-(1 as libc::c_int as isize))
        })
...
unsafe extern "C" fn label(
    ...
    while !e.is_null() {
        *edgecnt += 1 as libc::c_int;
        if (*e).node == n {
            e = if ((*(e as *mut Agobj_t)).tag).objtype() as libc::c_int == 3 as libc::c_int {
                e.offset(-(1 as libc::c_int as isize))
            } else {
                e.offset(1 as libc::c_int as isize)
            };
...
unsafe extern "C" fn process(mut G: *mut Agraph_t) {
    ...
    if Verbose != 0 {
        fprintf(
            ...
            nc,
            state.Comp,
            state.N_nodes_in_nontriv_SCC as libc::c_double / agnnodes(G) as libc::c_double,
            Maxdegree,
...
unsafe extern "C" fn scanArgs(mut argc: libc::c_int, mut argv: *mut *mut libc::c_char) {
    ...
    loop {
        c = getopt(
            argc,
            argv,
            b":o:sdvS?\0" as *const u8 as *const libc::c_char,
        );
