...
#[no_mangle]
pub unsafe extern "C" fn gvtextlayout_select(mut gvc: *mut GVC_t) -> libc::c_int {
    ...
    plugin = gvplugin_load(
        gvc,
        API_textlayout,
        b"textlayout\0" as *const u8 as *const libc::c_char,
    );
    if !plugin.is_null() {
        typeptr = (*plugin).typeptr;
        let ref mut fresh0 = (*gvc).textlayout.engine;
        *fresh0 = (*typeptr).engine as *mut gvtextlayout_engine_t;
        return 300 as libc::c_int;
    }
    return 999 as libc::c_int;
}
#[no_mangle]
pub unsafe extern "C" fn gvtextlayout(
    mut gvc: *mut GVC_t,
    mut span: *mut textspan_t,
    mut fontpath: *mut *mut libc::c_char,
) -> bool {
    let mut gvte: *mut gvtextlayout_engine_t = (*gvc).textlayout.engine;
    if !gvte.is_null() && ((*gvte).textlayout).is_some() {
        return ((*gvte).textlayout).expect("non-null function pointer")(span, fontpath);
    }
