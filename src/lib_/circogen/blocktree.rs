...
extern "C" {
    ...
    fn mkBlock(_: *mut Agraph_t) -> *mut block_t;
    fn freeBlock(sp: *mut block_t);
    fn blockSize(sp: *mut block_t) -> libc::c_int;
    fn initBlocklist(_: *mut blocklist_t);
    fn appendBlock(sp: *mut blocklist_t, sn: *mut block_t);
    fn insertBlock(sp: *mut blocklist_t, sn: *mut block_t);
}
...
#[repr(C)]
pub struct estack {
    pub top: *mut Agedge_t,
    pub sz: libc::c_int,
}
unsafe extern "C" fn addNode(mut bp: *mut block_t, mut n: *mut Agnode_t) {
    agsubnode((*bp).sub_graph, n, 1 as libc::c_int);
    let ref mut fresh0 =
        (*((*((*(n as *mut Agobj_t)).data as *mut Agnodeinfo_t)).alg as *mut cdata)).block;
    *fresh0 = bp;
}
unsafe extern "C" fn makeBlockGraph(
    mut g: *mut Agraph_t,
    mut state: *mut circ_state,
) -> *mut Agraph_t {
    let mut name: [libc::c_char; 128] = [0; 128];
    let mut subg: *mut Agraph_t = 0 as *mut Agraph_t;
    let ref mut fresh1 = (*state).blockCount;
    let fresh2 = *fresh1;
    *fresh1 = *fresh1 + 1;
    snprintf(
        name.as_mut_ptr(),
        ::std::mem::size_of::<[libc::c_char; 128]>() as libc::c_ulong,
        b"_block_%d\0" as *const u8 as *const libc::c_char,
        fresh2,
    );
    subg = agsubg(g, name.as_mut_ptr(), 1 as libc::c_int);
    agbindrec(
...
unsafe extern "C" fn makeBlock(mut g: *mut Agraph_t, mut state: *mut circ_state) -> *mut block_t {
    let mut subg: *mut Agraph_t = makeBlockGraph(g, state);
    let mut bp: *mut block_t = mkBlock(subg);
    return bp;
}
unsafe extern "C" fn push(mut s: *mut estack, mut e: *mut Agedge_t) {
    let ref mut fresh3 =
        (*((*((*(e as *mut Agobj_t)).data as *mut Agedgeinfo_t)).alg as *mut edata)).next;
    *fresh3 = (*s).top;
    let ref mut fresh4 = (*s).top;
    *fresh4 = e;
    (*s).sz += 1 as libc::c_int;
}
unsafe extern "C" fn pop(mut s: *mut estack) -> *mut Agedge_t {
    let mut top: *mut Agedge_t = (*s).top;
    if !top.is_null() {
        if (*s).sz > 0 as libc::c_int {
        } else {
            __assert_fail(
                b"s->sz > 0\0" as *const u8 as *const libc::c_char,
                b"blocktree.c\0" as *const u8 as *const libc::c_char,
                59 as libc::c_int as libc::c_uint,
                (*::std::mem::transmute::<&[u8; 24], &[libc::c_char; 24]>(
                    b"Agedge_t *pop(estack *)\0",
                ))
                .as_ptr(),
            );
        }
        let ref mut fresh5 = (*s).top;
        *fresh5 =
            (*((*((*(top as *mut Agobj_t)).data as *mut Agedgeinfo_t)).alg as *mut edata)).next;
        (*s).sz -= 1 as libc::c_int;
    } else {
        __assert_fail(
...
unsafe extern "C" fn dfs(
    ...
    mut u: *mut Agnode_t,
    mut state: *mut circ_state,
    mut isRoot: libc::c_int,
    mut stk: *mut estack,
) {
    let mut e: *mut Agedge_t = 0 as *mut Agedge_t;
    let mut v: *mut Agnode_t = 0 as *mut Agnode_t;
    let ref mut fresh6 = (*state).orderCount;
    let fresh7 = *fresh6;
    *fresh6 = *fresh6 + 1;
    let ref mut fresh8 = (*((*((*(u as *mut Agobj_t)).data as *mut Agnodeinfo_t)).alg
        as *mut cdata))
        .u
        .bc
        .val;
    *fresh8 = fresh7;
    (*((*((*(u as *mut Agobj_t)).data as *mut Agnodeinfo_t)).alg as *mut cdata))
        .u
        .bc
        .low_val = *fresh8;
    e = agfstedge(g, u);
    while !e.is_null() {
        ...
        if v == u {
            ...
            .node;
            if (*((*((*(e as *mut Agobj_t)).data as *mut Agedgeinfo_t)).alg as *mut edata)).order
                == 0
            {
                (*((*((*(e as *mut Agobj_t)).data as *mut Agedgeinfo_t)).alg as *mut edata))
                    .order = -(1 as libc::c_int);
            }
        } else if (*((*((*(e as *mut Agobj_t)).data as *mut Agedgeinfo_t)).alg as *mut edata)).order
            == 0
        {
            (*((*((*(e as *mut Agobj_t)).data as *mut Agedgeinfo_t)).alg as *mut edata)).order =
                1 as libc::c_int;
        }
        if (*((*((*(v as *mut Agobj_t)).data as *mut Agnodeinfo_t)).alg as *mut cdata))
            .u
            .bc
            .val
            == 0 as libc::c_int
        {
            let ref mut fresh9 =
                (*((*((*(v as *mut Agobj_t)).data as *mut Agnodeinfo_t)).alg as *mut cdata)).parent;
            *fresh9 = u;
            push(stk, e);
            dfs(g, v, state, 0 as libc::c_int, stk);
            (*((*((*(u as *mut Agobj_t)).data as *mut Agnodeinfo_t)).alg as *mut cdata))
                .u
                .bc
                .low_val = if (*((*((*(u as *mut Agobj_t)).data as *mut Agnodeinfo_t)).alg
                as *mut cdata))
                .u
                .bc
                .low_val
                < (*((*((*(v as *mut Agobj_t)).data as *mut Agnodeinfo_t)).alg as *mut cdata))
                    .u
                    .bc
                    .low_val
            {
            ...
            if (*((*((*(v as *mut Agobj_t)).data as *mut Agnodeinfo_t)).alg as *mut cdata))
                ...
                >= (*((*((*(u as *mut Agobj_t)).data as *mut Agnodeinfo_t)).alg as *mut cdata))
                    ...
                    .val
            {
                let mut block: *mut block_t = 0 as *mut block_t;
                let mut np: *mut Agnode_t = 0 as *mut Agnode_t;
                let mut ep: *mut Agedge_t = 0 as *mut Agedge_t;
                loop {
                    ep = pop(stk);
                    if (*((*((*(ep as *mut Agobj_t)).data as *mut Agedgeinfo_t)).alg as *mut edata))
                        .order
                        == 1 as libc::c_int
                    {
                    ...
                    if ((*((*((*(np as *mut Agobj_t)).data as *mut Agnodeinfo_t)).alg
                        as *mut cdata))
                        .block)
                        .is_null()
                    {
                ...
                if !block.is_null() {
                    if ((*((*((*(u as *mut Agobj_t)).data as *mut Agnodeinfo_t)).alg
                        as *mut cdata))
                        .block)
                        .is_null()
                        && blockSize(block) > 1 as libc::c_int
                    {
                        addNode(block, u);
                    }
                    if isRoot != 0
                        && (*((*((*(u as *mut Agobj_t)).data as *mut Agnodeinfo_t)).alg
                            as *mut cdata))
                            .block
                            == block
                    {
                        insertBlock(&mut (*state).bl, block);
                    } else {
                        appendBlock(&mut (*state).bl, block);
                    }
                }
            }
        } else if (*((*((*(u as *mut Agobj_t)).data as *mut Agnodeinfo_t)).alg as *mut cdata))
            .parent
            != v
        {
            (*((*((*(u as *mut Agobj_t)).data as *mut Agnodeinfo_t)).alg as *mut cdata))
    ...
    if isRoot != 0
        && ((*((*((*(u as *mut Agobj_t)).data as *mut Agnodeinfo_t)).alg as *mut cdata)).block)
            .is_null()
    {
        let mut block_0: *mut block_t = makeBlock(g, state);
        addNode(block_0, u);
        insertBlock(&mut (*state).bl, block_0);
    }
}
unsafe extern "C" fn find_blocks(mut g: *mut Agraph_t, mut state: *mut circ_state) {
    let mut n: *mut Agnode_t = 0 as *mut Agnode_t;
    let mut root: *mut Agnode_t = 0 as *mut Agnode_t;
    let mut stk: estack = estack {
        top: 0 as *mut Agedge_t,
        sz: 0,
    };
    if !((*state).rootname).is_null() {
        root = agnode(g, (*state).rootname, 0 as libc::c_int);
    }
    if root.is_null() && !((*state).N_root).is_null() {
        n = agfstnode(g);
        while !n.is_null() {
            if late_bool(
    ...
    stk.sz = 0 as libc::c_int;
    stk.top = 0 as *mut Agedge_t;
    dfs(g, root, state, 1 as libc::c_int, &mut stk);
}
#[no_mangle]
pub unsafe extern "C" fn createBlocktree(
    mut g: *mut Agraph_t,
    mut state: *mut circ_state,
) -> *mut block_t {
    let mut bp: *mut block_t = 0 as *mut block_t;
    let mut next: *mut block_t = 0 as *mut block_t;
    let mut root: *mut block_t = 0 as *mut block_t;
    ...
    while !bp.is_null() {
        ...
        let mut child: *mut Agnode_t = 0 as *mut Agnode_t;
        let mut subg: *mut Agraph_t = (*bp).sub_graph;
        n = agfstnode(subg);
        child = n;
        min = (*((*((*(n as *mut Agobj_t)).data as *mut Agnodeinfo_t)).alg as *mut cdata))
            .u
            .bc
            .val;
        parent =
            (*((*((*(n as *mut Agobj_t)).data as *mut Agnodeinfo_t)).alg as *mut cdata)).parent;
        n = agnxtnode(subg, n);
        while !n.is_null() {
                ...
                < min
            {
                ...
                min = (*((*((*(n as *mut Agobj_t)).data as *mut Agnodeinfo_t)).alg as *mut cdata))
                    ...
                    .bc
                    .val;
                parent = (*((*((*(n as *mut Agobj_t)).data as *mut Agnodeinfo_t)).alg
                    as *mut cdata))
                    .parent;
            }
            n = agnxtnode(subg, n);
        }
        (*((*((*(parent as *mut Agobj_t)).data as *mut Agnodeinfo_t)).alg as *mut cdata)).flags |=
            (1 as libc::c_int) << 3 as libc::c_int;
        let ref mut fresh10 = (*bp).child;
        *fresh10 = child;
        next = (*bp).next;
        appendBlock(
            &mut (*(*((*((*(parent as *mut Agobj_t)).data as *mut Agnodeinfo_t)).alg
                as *mut cdata))
                .block)
                .children,
            bp,
    ...
    initBlocklist(&mut (*state).bl);
    return root;
}
#[no_mangle]
pub unsafe extern "C" fn freeBlocktree(mut bp: *mut block_t) {
    let mut child: *mut block_t = 0 as *mut block_t;
    let mut next: *mut block_t = 0 as *mut block_t;
    child = (*bp).children.first;
    while !child.is_null() {
        next = (*child).next;
        freeBlocktree(child);
        child = next;
