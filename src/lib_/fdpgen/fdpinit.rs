...
extern "C" {
    ...
    fn processClusterEdges(g: *mut graph_t);
...
unsafe extern "C" fn initialPositions(mut g: *mut graph_t) {
    let mut i: libc::c_int = 0;
    let mut np: *mut node_t = 0 as *mut node_t;
    let mut possym: *mut attrsym_t = 0 as *mut attrsym_t;
    let mut pinsym: *mut attrsym_t = 0 as *mut attrsym_t;
    ...
    loop {
        ...
        p = agxget(np as *mut libc::c_void, possym);
            ...
            ) >= 2 as libc::c_int
            {
                if PSinputscale > 0.0f64 {
                    ...
                    while j < 2 as libc::c_int {
                        *pvec.offset(j as isize) = *pvec.offset(j as isize) / PSinputscale;
                        j += 1;
                    }
                }
                ...
                if c as libc::c_int == '!' as i32
                    || !pinsym.is_null()
                        && mapbool(agxget(np as *mut libc::c_void, pinsym)) as libc::c_int != 0
                {
                    (*((*(np as *mut Agobj_t)).data as *mut Agnodeinfo_t)).pinned =
                ...
                fprintf(
                    stderr,
                    b"Warning: node %s, position %s, expected two floats\n\0" as *const u8
...
unsafe extern "C" fn init_edge(mut e: *mut edge_t, mut E_len: *mut attrsym_t) {
    agbindrec(
    ...
    (*((*(e as *mut Agobj_t)).data as *mut Agedgeinfo_t)).dist =
        late_double(e as *mut libc::c_void, E_len, (*fdp_parms).K, 0.0f64);
    common_init_edge(e);
}
unsafe extern "C" fn init_node(mut n: *mut node_t) {
    common_init_node(n);
    let ref mut fresh0 = (*((*(n as *mut Agobj_t)).data as *mut Agnodeinfo_t)).pos;
    *fresh0 = gcalloc(
...
#[no_mangle]
pub unsafe extern "C" fn fdp_init_node_edge(mut g: *mut graph_t) {
    let mut E_len: *mut attrsym_t = 0 as *mut attrsym_t;
    ...
    nn = agnnodes(g);
    let ref mut fresh1 = (*((*(g as *mut Agobj_t)).data as *mut Agraphinfo_t)).neato_nlist;
    *fresh1 = gcalloc(
    ...
    while !n.is_null() {
        ...
        *fresh2 = n;
        let fresh3 = i;
        i = i + 1;
        (*((*(n as *mut Agobj_t)).data as *mut Agnodeinfo_t)).id = fresh3;
        n = agnxtnode(g, n);
    }
    E_len = agattr(
        g,
...
unsafe extern "C" fn cleanup_subgs(mut g: *mut graph_t) {
    ...
    while i <= (*((*(g as *mut Agobj_t)).data as *mut Agraphinfo_t)).n_cluster {
        ...
        if !((*((*(subg as *mut Agobj_t)).data as *mut Agraphinfo_t)).alg).is_null() {
            free(
                (*((*((*(subg as *mut Agobj_t)).data as *mut Agraphinfo_t)).alg as *mut gdata))
                    .ports as *mut libc::c_void,
            );
            free((*((*(subg as *mut Agobj_t)).data as *mut Agraphinfo_t)).alg);
        }
        cleanup_subgs(subg);
        i += 1;
    }
...
unsafe extern "C" fn fdp_cleanup_graph(mut g: *mut graph_t) {
    cleanup_subgs(g);
...
#[no_mangle]
pub unsafe extern "C" fn fdp_cleanup(mut g: *mut graph_t) {
    let mut n: *mut node_t = 0 as *mut node_t;
    let mut e: *mut edge_t = 0 as *mut edge_t;
    n = agfstnode(g);
    while !n.is_null() {
