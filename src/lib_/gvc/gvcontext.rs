...
static mut LibInfo: [*mut libc::c_char; 3] = [
    b"graphviz\0" as *const u8 as *const libc::c_char as *mut libc::c_char,
...
#[no_mangle]
pub unsafe extern "C" fn gvNEWcontext(
    mut builtins: *const lt_symlist_t,
    mut demand_loading: libc::c_int,
) -> *mut GVC_t {
    let mut gvc: *mut GVC_t = zmalloc(::std::mem::size_of::<GVC_t>() as libc::c_ulong)
        as *mut GVC_t;
    let ref mut fresh0 = (*gvc).common.info;
    *fresh0 = LibInfo.as_mut_ptr();
    let ref mut fresh1 = (*gvc).common.errorfn;
    *fresh1 = Some(agerrorf as unsafe extern "C" fn(*const libc::c_char, ...) -> ());
    let ref mut fresh2 = (*gvc).common.builtins;
    *fresh2 = builtins;
    (*gvc).common.demand_loading = demand_loading;
    return gvc;
}
#[no_mangle]
pub unsafe extern "C" fn gvFinalize(mut gvc: *mut GVC_t) {
    if !((*gvc).active_jobs).is_null() {
        gvrender_end_job((*gvc).active_jobs);
    }
}
#[no_mangle]
pub unsafe extern "C" fn gvFreeContext(mut gvc: *mut GVC_t) -> libc::c_int {
    let mut gvg: *mut GVG_t = 0 as *mut GVG_t;
    let mut gvg_next: *mut GVG_t = 0 as *mut GVG_t;
    let mut package: *mut gvplugin_package_t = 0 as *mut gvplugin_package_t;
    let mut package_next: *mut gvplugin_package_t = 0 as *mut gvplugin_package_t;
    let mut api: *mut gvplugin_available_t = 0 as *mut gvplugin_available_t;
    let mut api_next: *mut gvplugin_available_t = 0 as *mut gvplugin_available_t;
    let mut num_apis: libc::c_uint = (1 as libc::c_int + 1 as libc::c_int
    ...
    loop {
        ...
        free((*package).path as *mut libc::c_void);
        free((*package).name as *mut libc::c_void);
        free(package as *mut libc::c_void);
    }
    gvjobs_delete(gvc);
    free((*gvc).config_path as *mut libc::c_void);
    free((*gvc).input_filenames as *mut libc::c_void);
    textfont_dict_close(gvc);
    i = 0 as libc::c_int as libc::c_uint;
    while i != num_apis {
        api = (*gvc).apis[i as usize];
        while !api.is_null() {
            api_next = (*api).next;
            free((*api).typestr as *mut libc::c_void);
            free(api as *mut libc::c_void);
            api = api_next;
        }
        i = i.wrapping_add(1);
    }
    free(gvc as *mut libc::c_void);
    return graphviz_errors + agerrors();
}
#[no_mangle]
pub unsafe extern "C" fn gvCloneGVC(mut gvc0: *mut GVC_t) -> *mut GVC_t {
    let mut gvc: *mut GVC_t = zmalloc(::std::mem::size_of::<GVC_t>() as libc::c_ulong)
        as *mut GVC_t;
    (*gvc).common = (*gvc0).common;
    memcpy(
        &mut (*gvc).apis as *mut [*mut gvplugin_available_t; 5] as *mut libc::c_void,
        &mut (*gvc0).apis as *mut [*mut gvplugin_available_t; 5] as *const libc::c_void,
        ::std::mem::size_of::<[*mut gvplugin_available_t; 5]>() as libc::c_ulong,
    );
    memcpy(
        &mut (*gvc).api as *mut [*mut gvplugin_available_t; 5] as *mut libc::c_void,
        &mut (*gvc0).api as *mut [*mut gvplugin_available_t; 5] as *const libc::c_void,
        ::std::mem::size_of::<[*mut gvplugin_available_t; 5]>() as libc::c_ulong,
    );
    let ref mut fresh3 = (*gvc).packages;
    *fresh3 = (*gvc0).packages;
    return gvc;
}
#[no_mangle]
pub unsafe extern "C" fn gvFreeCloneGVC(mut gvc: *mut GVC_t) {
    gvjobs_delete(gvc);
    free(gvc as *mut libc::c_void);
}
