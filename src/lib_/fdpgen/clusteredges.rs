...
extern "C" {
    ...
    fn Pobsopen(
        obstacles: *mut *mut Ppoly_t,
        n_obstacles: libc::c_int,
    ) -> *mut vconfig_t;
...
unsafe extern "C" fn makeClustObs(
    mut g: *mut graph_t,
    mut pm: *mut expand_t,
) -> *mut Ppoly_t {
    let mut obs: *mut Ppoly_t = zmalloc(
        ::std::mem::size_of::<Ppoly_t>() as libc::c_ulong,
    ) as *mut Ppoly_t;
...
unsafe extern "C" fn addGraphObjs(
    ...
    while !n.is_null() {
        if (*((*(n as *mut Agobj_t)).data as *mut Agnodeinfo_t)).clust == g
            && n != tex as *mut node_t && n != hex as *mut node_t
...
unsafe extern "C" fn raiseLevel(
    ...
    while i > minlvl {
        ...
        g = (*((*((*(g as *mut Agobj_t)).data as *mut Agraphinfo_t)).alg as *mut gdata))
            .parent;
        i -= 1;
    }
    *gp = ex as *mut graph_t;
}
unsafe extern "C" fn objectList(
    mut ep: *mut edge_t,
    mut pm: *mut expand_t,
) -> *mut objlist {
    let mut h: *mut node_t = (*if ((*(ep as *mut Agobj_t)).tag).objtype() as libc::c_int
        == 2 as libc::c_int
    {
        ep
    } else {
        ...
        .node;
    let mut t: *mut node_t = (*if ((*(ep as *mut Agobj_t)).tag).objtype() as libc::c_int
        == 3 as libc::c_int
    {
        ep
    } else {
        ...
        .node;
    let mut hg: *mut graph_t = (*((*(h as *mut Agobj_t)).data as *mut Agnodeinfo_t))
        .clust;
    let mut tg: *mut graph_t = (*((*(t as *mut Agobj_t)).data as *mut Agnodeinfo_t))
        .clust;
    ...
    let mut list: *mut objlist = zmalloc(
        ::std::mem::size_of::<objlist>() as libc::c_ulong,
    ) as *mut objlist;
    if (*((*(h as *mut Agobj_t)).data as *mut Agnodeinfo_t)).clustnode {
        hex = hg as *mut libc::c_void;
        hg = (*((*((*(hg as *mut Agobj_t)).data as *mut Agraphinfo_t)).alg
            as *mut gdata))
            .parent;
    } else {
    ...
    if (*((*(t as *mut Agobj_t)).data as *mut Agnodeinfo_t)).clustnode {
        ...
        tg = (*((*((*(tg as *mut Agobj_t)).data as *mut Agraphinfo_t)).alg
            as *mut gdata))
            .parent;
    } else {
        tex = t as *mut libc::c_void;
    }
    hlevel = (*((*((*(hg as *mut Agobj_t)).data as *mut Agraphinfo_t)).alg
        as *mut gdata))
        .level;
    tlevel = (*((*((*(tg as *mut Agobj_t)).data as *mut Agraphinfo_t)).alg
        as *mut gdata))
        .level;
    if hlevel > tlevel {
...
pub unsafe extern "C" fn compoundEdges(
    ...
    while !n.is_null() {
        e = agfstout(g, n);
        while !e.is_null() {
            head = (*if ((*(e as *mut Agobj_t)).tag).objtype() as libc::c_int
                == 2 as libc::c_int
            {
                e
            } else {
                ...
                .node;
            if n == head
                && (*((*(e as *mut Agobj_t)).data as *mut Agedgeinfo_t)).count
                    as libc::c_int != 0
            {
                if P.is_null() {
                    P = zmalloc(::std::mem::size_of::<path>() as libc::c_ulong)
                        as *mut path;
                    let ref mut fresh6 = (*P).boxes;
                    *fresh6 = gcalloc(
                        (agnnodes(g)
                            + 20 as libc::c_int * 2 as libc::c_int * 9 as libc::c_int)
                            as size_t,
                ...
                if Plegal_arrangement((*objl).obs, (*objl).cnt) != 0 {
                    ...
                    if vconfig.is_null() {
                        ...
                        while !e0.is_null() {
                            ...
                            e0 = (*((*(e0 as *mut Agobj_t)).data as *mut Agedgeinfo_t))
                                .to_virt;
                        }
                        resetObjlist(objl);
                    }
                    ...
                    let mut pack: libc::c_int = getPack(
                        g,
                        8 as libc::c_int,
                        8 as libc::c_int,
                    );
                    ...
                    if pack as libc::c_float <= (*pm).x
                        || pack as libc::c_float <= (*pm).y
                    {
                        agerr(
                            AGPREV,
