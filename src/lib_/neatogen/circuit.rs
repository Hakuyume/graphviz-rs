...
#[no_mangle]
pub unsafe extern "C" fn solveCircuit(
    mut nG: libc::c_int,
    mut Gm: *mut *mut libc::c_double,
    mut Gm_inv: *mut *mut libc::c_double,
) -> libc::c_int {
    ...
    if Verbose != 0 {
        fprintf(
            stderr,
            b"Calculating circuit model\0" as *const u8 as *const libc::c_char,
        );
    }
    i = 0 as libc::c_int;
    while i < nG {
        ...
        while j < nG {
            if i != j {
                sum += *(*Gm.offset(i as isize)).offset(j as isize);
            }
            j += 1;
        }
        *(*Gm.offset(i as isize)).offset(i as isize) = -sum;
        i += 1;
    }
    return matinv(Gm, Gm_inv, nG - 1 as libc::c_int);
}
#[no_mangle]
pub unsafe extern "C" fn circuit_model(mut g: *mut graph_t, mut nG: libc::c_int) -> libc::c_int {
    ...
    while !v.is_null() {
        ...
        while !e.is_null() {
                ...
                == 2 as libc::c_int
            {
                ...
                .seq() as libc::c_long;
            if !(i == j) {
                let ref mut fresh0 = *(*Gm.offset(j as isize)).offset(i as isize);
                *fresh0 = -1.0f64 / (*((*(e as *mut Agobj_t)).data as *mut Agedgeinfo_t)).dist;
                *(*Gm.offset(i as isize)).offset(j as isize) = *fresh0;
            }
            e = agnxtedge(g, e, v);
        }
        v = agnxtnode(g, v);
    ...
    if rv != 0 {
        ...
        while i < nG as libc::c_long {
            j = 0 as libc::c_int as libc::c_long;
            while j < nG as libc::c_long {
                *(*((*((*(g as *mut Agobj_t)).data as *mut Agraphinfo_t)).dist)
                    .offset(i as isize))
                .offset(j as isize) = *(*Gm_inv.offset(i as isize)).offset(i as isize)
