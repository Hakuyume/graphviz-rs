...
extern "C" {
    ...
    fn rsvg_handle_set_dpi(handle: *mut RsvgHandle, dpi: libc::c_double);
...
#[derive(Copy, Clone)]
#[repr(C)]
pub struct _RsvgHandle {
    pub parent: GObject,
    pub _abi_padding: [gpointer; 16],
}
pub type RsvgHandle = _RsvgHandle;
pub type C2RustUnnamed_4 = libc::c_uint;
pub const FORMAT_SVG_CAIRO: C2RustUnnamed_4 = 0;
unsafe extern "C" fn gvloadimage_rsvg_free(mut us: *mut usershape_t) {
    rsvg_handle_close((*us).data as *mut RsvgHandle, 0 as *mut *mut GError);
}
unsafe extern "C" fn gvloadimage_rsvg_load(
    mut job: *mut GVJ_t,
    mut us: *mut usershape_t,
) -> *mut RsvgHandle {
    let mut rsvgh: *mut RsvgHandle = 0 as *mut RsvgHandle;
    let mut fileBuf: *mut guchar = 0 as *mut guchar;
    ...
    if !job.is_null() {
        ...
        __assert_fail(
            ...
            b"gvloadimage_rsvg.c\0" as *const u8 as *const libc::c_char,
            47 as libc::c_int as libc::c_uint,
            (*::std::mem::transmute::<&[u8; 58], &[libc::c_char; 58]>(
                b"RsvgHandle *gvloadimage_rsvg_load(GVJ_t *, usershape_t *)\0",
            ))
            .as_ptr(),
        );
    }
    ...
    if !((*us).data).is_null() {
        if (*us).datafree
            == Some(gvloadimage_rsvg_free as unsafe extern "C" fn(*mut usershape_t) -> ())
        {
            rsvgh = (*us).data as *mut RsvgHandle;
        } else {
        ...
        match (*us).type_0 as libc::c_uint {
            8 => {
                ...
                if rsvgh.is_null() {
                    fprintf(
                        stderr,
                        b"rsvg_handle_new_from_file returned an error: %s\n\0" as *const u8
                ...
                fd = fileno((*us).f);
                fstat(fd, &mut stbuf);
                fileSize = stbuf.st_size as size_t;
                fileBuf = calloc(
                    fileSize.wrapping_add(1 as libc::c_int as libc::c_ulong),
                    ::std::mem::size_of::<guchar>() as libc::c_ulong,
                ) as *mut guchar;
                if fileBuf.is_null() {
                    g_object_unref(rsvgh as gpointer);
                    return 0 as *mut RsvgHandle;
                }
                rewind((*us).f);
                if fread(
                ...
                if rsvg_handle_write(rsvgh, fileBuf as *const guchar, fileSize, &mut err)
                    == 0 as libc::c_int
                {
                    fprintf(
                        stderr,
                        b"rsvg_handle_write returned an error: %s\n\0" as *const u8
                            as *const libc::c_char,
                        (*err).message,
                    );
                    free(fileBuf as *mut libc::c_void);
                ...
                free(fileBuf as *mut libc::c_void);
                rsvg_handle_close(rsvgh, &mut err);
                rsvg_handle_set_dpi(rsvgh, 72 as libc::c_int as libc::c_double);
            }
            _ => {
                rsvgh = 0 as *mut RsvgHandle;
            }
        }
        if !rsvgh.is_null() {
            let ref mut fresh1 = (*us).data;
            *fresh1 = rsvgh as *mut libc::c_void;
            let ref mut fresh2 = (*us).datafree;
            *fresh2 = Some(gvloadimage_rsvg_free as unsafe extern "C" fn(*mut usershape_t) -> ());
        }
        gvusershape_file_release(us);
    }
    return rsvgh;
}
unsafe extern "C" fn gvloadimage_rsvg_cairo(
    mut job: *mut GVJ_t,
    mut us: *mut usershape_t,
    mut b: boxf,
    mut filled: bool,
) {
    let mut rsvgh: *mut RsvgHandle = gvloadimage_rsvg_load(job, us);
    ...
    if !rsvgh.is_null() {
        ...
        surface = cairo_svg_surface_create(
            ...
            (*us).w as libc::c_double,
            (*us).h as libc::c_double,
        );
...
#[no_mangle]
pub static mut gvloadimage_rsvg_types: [gvplugin_installed_t; 2] = unsafe {
    [
        {
            let mut init = gvplugin_installed_t {
                id: FORMAT_SVG_CAIRO as libc::c_int,
