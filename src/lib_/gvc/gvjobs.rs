...
static mut output_filename_job: *mut GVJ_t = 0 as *const GVJ_t as *mut GVJ_t;
static mut output_langname_job: *mut GVJ_t = 0 as *const GVJ_t as *mut GVJ_t;
#[no_mangle]
pub unsafe extern "C" fn gvjobs_output_filename(
    mut gvc: *mut GVC_t,
    mut name: *const libc::c_char,
) {
    if ((*gvc).jobs).is_null() {
        let ref mut fresh0 = (*gvc).jobs;
        *fresh0 = zmalloc(::std::mem::size_of::<GVJ_t>() as libc::c_ulong) as *mut GVJ_t;
        let ref mut fresh1 = (*gvc).job;
        *fresh1 = *fresh0;
        output_filename_job = *fresh1;
    } else if output_filename_job.is_null() {
        output_filename_job = (*gvc).jobs;
    } else {
        if ((*output_filename_job).next).is_null() {
            let ref mut fresh2 = (*output_filename_job).next;
            *fresh2 = zmalloc(::std::mem::size_of::<GVJ_t>() as libc::c_ulong) as *mut GVJ_t;
        }
        output_filename_job = (*output_filename_job).next;
    }
    let ref mut fresh3 = (*output_filename_job).output_filename;
    *fresh3 = name;
    let ref mut fresh4 = (*output_filename_job).gvc;
    *fresh4 = gvc;
}
#[no_mangle]
pub unsafe extern "C" fn gvjobs_output_langname(
    mut gvc: *mut GVC_t,
    mut name: *const libc::c_char,
) -> bool {
    if ((*gvc).jobs).is_null() {
        let ref mut fresh5 = (*gvc).jobs;
        *fresh5 = zmalloc(::std::mem::size_of::<GVJ_t>() as libc::c_ulong) as *mut GVJ_t;
        let ref mut fresh6 = (*gvc).job;
        *fresh6 = *fresh5;
        output_langname_job = *fresh6;
    } else if output_langname_job.is_null() {
        output_langname_job = (*gvc).jobs;
    } else {
        if ((*output_langname_job).next).is_null() {
            let ref mut fresh7 = (*output_langname_job).next;
            *fresh7 = zmalloc(::std::mem::size_of::<GVJ_t>() as libc::c_ulong) as *mut GVJ_t;
        }
        output_langname_job = (*output_langname_job).next;
    }
    let ref mut fresh8 = (*output_langname_job).output_langname;
    *fresh8 = name;
    let ref mut fresh9 = (*output_langname_job).gvc;
    *fresh9 = gvc;
    if !(gvplugin_load(gvc, API_device, name)).is_null() {
        return 1 as libc::c_int != 0;
    }
    return 0 as libc::c_int != 0;
}
#[no_mangle]
pub unsafe extern "C" fn gvjobs_first(mut gvc: *mut GVC_t) -> *mut GVJ_t {
    let ref mut fresh10 = (*gvc).job;
    *fresh10 = (*gvc).jobs;
    return *fresh10;
}
#[no_mangle]
pub unsafe extern "C" fn gvjobs_next(mut gvc: *mut GVC_t) -> *mut GVJ_t {
    let mut job: *mut GVJ_t = (*(*gvc).job).next;
    if !job.is_null() {
        if ((*job).output_langname).is_null() {
            let ref mut fresh11 = (*job).output_langname;
            *fresh11 = (*(*gvc).job).output_langname;
        }
    }
    let ref mut fresh12 = (*gvc).job;
    *fresh12 = job;
    return *fresh12;
}
#[no_mangle]
pub unsafe extern "C" fn gvNEWargvlist() -> *mut gv_argvlist_t {
    return zmalloc(::std::mem::size_of::<gv_argvlist_t>() as libc::c_ulong) as *mut gv_argvlist_t;
}
#[no_mangle]
pub unsafe extern "C" fn gv_argvlist_set_item(
    mut list: *mut gv_argvlist_t,
    mut index: libc::c_int,
    mut item: *mut libc::c_char,
) {
    if index >= (*list).alloc {
        (*list).alloc = index + 10 as libc::c_int;
        let ref mut fresh13 = (*list).argv;
        *fresh13 = grealloc(
            (*list).argv as *mut libc::c_void,
            ((*list).alloc as libc::c_ulong)
                .wrapping_mul(::std::mem::size_of::<*mut libc::c_char>() as libc::c_ulong),
        ) as *mut *mut libc::c_char;
    }
    let ref mut fresh14 = *((*list).argv).offset(index as isize);
    *fresh14 = item;
}
#[no_mangle]
pub unsafe extern "C" fn gv_argvlist_reset(mut list: *mut gv_argvlist_t) {
    free((*list).argv as *mut libc::c_void);
    let ref mut fresh15 = (*list).argv;
    *fresh15 = 0 as *mut *mut libc::c_char;
    (*list).alloc = 0 as libc::c_int;
    (*list).argc = 0 as libc::c_int;
}
#[no_mangle]
pub unsafe extern "C" fn gv_argvlist_free(mut list: *mut gv_argvlist_t) {
    free((*list).argv as *mut libc::c_void);
    free(list as *mut libc::c_void);
}
#[no_mangle]
pub unsafe extern "C" fn gvjobs_delete(mut gvc: *mut GVC_t) {
    let mut job: *mut GVJ_t = 0 as *mut GVJ_t;
    let mut j: *mut GVJ_t = 0 as *mut GVJ_t;
    job = (*gvc).jobs;
    loop {
        j = job;
        if j.is_null() {
        ...
        job = (*job).next;
        gv_argvlist_reset(&mut (*j).selected_obj_attributes);
        gv_argvlist_reset(&mut (*j).selected_obj_type_name);
        free((*j).active_tooltip as *mut libc::c_void);
        free((*j).selected_href as *mut libc::c_void);
        free(j as *mut libc::c_void);
    }
    output_langname_job = 0 as *mut GVJ_t;
    output_filename_job = output_langname_job;
    let ref mut fresh16 = (*gvc).active_jobs;
    *fresh16 = output_filename_job;
    let ref mut fresh17 = (*gvc).job;
    *fresh17 = *fresh16;
    let ref mut fresh18 = (*gvc).jobs;
    *fresh18 = *fresh17;
