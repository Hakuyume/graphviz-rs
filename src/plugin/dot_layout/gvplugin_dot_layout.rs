...
extern "C" {
    static mut gvlayout_dot_layout: [gvplugin_installed_t; 0];
}
    ...
    [
        {
            let mut init = gvplugin_api_t {
                api: API_layout,
                types: gvlayout_dot_layout.as_ptr() as *mut _,
            };
            init
        },
        {
...
#[no_mangle]
pub static mut gvplugin_dot_layout_LTX_library: gvplugin_library_t = unsafe {
    {
        let mut init = gvplugin_library_t {
            packagename: b"dot_layout\0" as *const u8 as *const libc::c_char as *mut libc::c_char,
