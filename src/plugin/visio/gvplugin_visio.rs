...
extern "C" {
    static mut gvdevice_vdx_types: [gvplugin_installed_t; 0];
    static mut gvrender_vdx_types: [gvplugin_installed_t; 0];
}
    ...
    [
        {
            let mut init = gvplugin_api_t {
                api: API_device,
                types: gvdevice_vdx_types.as_ptr() as *mut _,
            };
            init
        },
        {
            let mut init = gvplugin_api_t {
                api: API_render,
                types: gvrender_vdx_types.as_ptr() as *mut _,
            };
            init
        },
        {
...
#[no_mangle]
pub static mut gvplugin_visio_LTX_library: gvplugin_library_t = unsafe {
    {
        let mut init = gvplugin_library_t {
            packagename: b"visio\0" as *const u8 as *const libc::c_char as *mut libc::c_char,
