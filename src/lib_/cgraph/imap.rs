...
pub unsafe extern "C" fn aginternalmaplookup(
    ...
    if !d.is_null() {
        ...
        if !search_str.is_null() {
            ...
            sym = (Some(((*(d as *mut Dt_t)).searchf).expect("non-null function pointer")))
...
pub unsafe extern "C" fn aginternalmapinsert(
    ...
    ent = agalloc(g, ::std::mem::size_of::<IMapEntry_t>() as libc::c_ulong) as *mut IMapEntry_t;
    (*ent).id = id;
...
pub unsafe extern "C" fn aginternalmapdelete(
    ...
    if !isym.is_null() {
        (Some(
            ((*(*((*(*g).clos).lookup_by_name)
                .as_mut_ptr()
                .offset(objtype as isize) as *mut Dt_t))
                .searchf)
                .expect("non-null function pointer"),
        ))
        .expect("non-null function pointer")(
    ...
    while i < 3 as libc::c_int {
        if !(*d_name.offset(i as isize)).is_null() {
            ...
            while !sym.is_null() {
                ...
                if *((*sym).str_0).offset(0 as libc::c_int as isize) as libc::c_int == '%' as i32 {
