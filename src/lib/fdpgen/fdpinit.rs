        ...
        if *p.offset(0 as libc::c_int as isize) != 0 {
            ...
            ) >= 2 as libc::c_int
            {
                if PSinputscale > 0.0f64 {
                    ...
                    while j < 2 as libc::c_int {
                        *pvec
                            .offset(
                                j as isize,
                            ) = *pvec.offset(j as isize) / PSinputscale;
                        j += 1;
                    }
                }
                ...
                if c as libc::c_int == '!' as i32
                    || !pinsym.is_null()
                        && mapbool(agxget(np as *mut libc::c_void, pinsym))
                            as libc::c_int != 0
                {
                    (*((*(np as *mut Agobj_t)).data as *mut Agnodeinfo_t))
...
unsafe extern "C" fn init_edge(mut e: *mut edge_t, mut E_len: *mut attrsym_t) {
    ...
    (*((*(e as *mut Agobj_t)).data as *mut Agedgeinfo_t))
        .dist = late_double(e as *mut libc::c_void, E_len, (*fdp_parms).K, 0.0f64);
    common_init_edge(e);
}
...
pub unsafe extern "C" fn fdp_init_node_edge(mut g: *mut graph_t) {
    ...
    nn = agnnodes(g);
    let ref mut fresh1 = (*((*(g as *mut Agobj_t)).data as *mut Agraphinfo_t))
        .neato_nlist;
    *fresh1 = gcalloc(
