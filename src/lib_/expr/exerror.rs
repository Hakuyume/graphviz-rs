...
extern "C" {
    ...
    fn excontext(_: *mut Expr_t, _: *mut libc::c_char, _: libc::c_int) -> *mut libc::c_char;
...
unsafe extern "C" fn make_msg(
    ...
    if r < 0 as libc::c_int {
        return strdup(b"malformed format\0" as *const u8 as *const libc::c_char);
    }
    len = (len as libc::c_ulong)
        .wrapping_add((r as size_t).wrapping_add(1 as libc::c_int as libc::c_ulong))
        as size_t as size_t;
    let mut s: *mut libc::c_char = malloc(len) as *mut libc::c_char;
    if s.is_null() {
        return 0 as *mut libc::c_char;
    }
    let mut offset: libc::c_int = snprintf(
        s,
        len,
        b"%s\n -- \0" as *const u8 as *const libc::c_char,
        buf.as_mut_ptr(),
    ...
    if offset > 0 as libc::c_int {
        ...
        __assert_fail(
            ...
            b"exerror.c\0" as *const u8 as *const libc::c_char,
            55 as libc::c_int as libc::c_uint,
            (*::std::mem::transmute::<&[u8; 53], &[libc::c_char; 53]>(
                b"char *make_msg(const char *, struct __va_list_tag *)\0",
            ))
            .as_ptr(),
        );
    }
...
#[no_mangle]
pub unsafe extern "C" fn exerror(mut format: *const libc::c_char, mut args: ...) {
    if ((*(*expr.program).disc).errorf).is_some() && (*expr.program).errors == 0 {
        let mut ap: ::std::ffi::VaListImpl;
        (*expr.program).errors = 1 as libc::c_int;
        ap = args.clone();
        let mut s: *mut libc::c_char = make_msg(format, ap.as_va_list());
        (Some(((*(*expr.program).disc).errorf).expect("non-null function pointer")))
            ...
            if !s.is_null() {
                ...
                b"out of space\0" as *const u8 as *const libc::c_char
            },
        );
        free(s as *mut libc::c_void);
    } else if (*(*expr.program).disc).flags
        & ((1 as libc::c_int) << 2 as libc::c_int) as libc::c_ulong
        != 0
    {
...
#[no_mangle]
pub unsafe extern "C" fn exwarn(mut format: *const libc::c_char, mut args: ...) {
    if ((*(*expr.program).disc).errorf).is_some() {
        let mut ap: ::std::ffi::VaListImpl;
        ap = args.clone();
        let mut s: *mut libc::c_char = make_msg(format, ap.as_va_list());
        (Some(((*(*expr.program).disc).errorf).expect("non-null function pointer")))
