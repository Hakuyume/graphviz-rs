...
extern "C" {
    ...
    pub type GVC_s;
    pub type _PopplerDocument;
    pub type _PopplerPage;
    pub type _cairo;
    ...
    fn g_build_filename(first_element: *const gchar, _: ...) -> *mut gchar;
    ...
    fn poppler_document_get_n_pages(document: *mut PopplerDocument) -> libc::c_int;
    ...
    fn poppler_page_render(page: *mut PopplerPage, cairo: *mut cairo_t);
}
...
pub const FORMAT_PDF_CAIRO: C2RustUnnamed_4 = 0;
unsafe extern "C" fn gvloadimage_poppler_free(mut us: *mut usershape_t) {
    g_object_unref((*us).data);
}
unsafe extern "C" fn gvloadimage_poppler_load(
    mut job: *mut GVJ_t,
    mut us: *mut usershape_t,
) -> *mut PopplerDocument {
    let mut document: *mut PopplerDocument = 0 as *mut PopplerDocument;
    let mut error: *mut GError = 0 as *mut GError;
    let mut absolute: *mut gchar = 0 as *mut gchar;
    let mut uri: *mut gchar = 0 as *mut gchar;
    let mut num_pages: libc::c_int = 0;
    if !job.is_null() {
    } else {
        __assert_fail(
            b"job\0" as *const u8 as *const libc::c_char,
            b"gvloadimage_poppler.c\0" as *const u8 as *const libc::c_char,
            41 as libc::c_int as libc::c_uint,
            (*::std::mem::transmute::<&[u8; 66], &[libc::c_char; 66]>(
                b"PopplerDocument *gvloadimage_poppler_load(GVJ_t *, usershape_t *)\0",
            ))
            .as_ptr(),
        );
    }
    ...
    if !((*us).data).is_null() {
        if (*us).datafree
            == Some(gvloadimage_poppler_free as unsafe extern "C" fn(*mut usershape_t) -> ())
        {
            document = (*us).data as *mut PopplerDocument;
        } else {
    ...
    if document.is_null() {
        if !gvusershape_file_access(us) {
            return 0 as *mut PopplerDocument;
        }
        match (*us).type_0 as libc::c_uint {
            5 => {
                if g_path_is_absolute((*us).name) != 0 {
                    absolute = g_strdup((*us).name);
                } else {
                    let mut dir: *mut gchar = g_get_current_dir();
                    absolute = g_build_filename(dir, (*us).name, 0 as *mut libc::c_void);
                    free(dir as *mut libc::c_void);
                }
                uri = g_filename_to_uri(absolute, 0 as *const gchar, &mut error);
                ...
                document =
                    poppler_document_new_from_file(uri, 0 as *const libc::c_char, &mut error);
                if document.is_null() {
                    printf(
                        b"%s\n\0" as *const u8 as *const libc::c_char,
                        (*error).message,
                ...
                if num_pages < 1 as libc::c_int {
                    printf(
                        b"poppler fail: num_pages %d,  must be at least 1\0" as *const u8
        ...
        if !document.is_null() {
            ...
            *fresh2 = document as *mut libc::c_void;
...
unsafe extern "C" fn gvloadimage_poppler_cairo(
    mut job: *mut GVJ_t,
    mut us: *mut usershape_t,
    mut b: boxf,
    mut filled: bool,
) {
    let mut document: *mut PopplerDocument = gvloadimage_poppler_load(job, us);
    let mut page: *mut PopplerPage = 0 as *mut PopplerPage;
    let mut cr: *mut cairo_t = (*job).context as *mut cairo_t;
    let mut surface: *mut cairo_surface_t = 0 as *mut cairo_surface_t;
    if !document.is_null() {
        page = poppler_document_get_page(document, 0 as libc::c_int);
        cairo_save(cr);
...
#[no_mangle]
pub static mut gvloadimage_poppler_types: [gvplugin_installed_t; 2] = unsafe {
    [
        {
            let mut init = gvplugin_installed_t {
                id: FORMAT_PDF_CAIRO as libc::c_int,
