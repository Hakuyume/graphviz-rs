...
unsafe extern "C" fn init_nodelistitem(mut n: *mut Agnode_t) -> *mut nodelistitem_t {
    let mut p: *mut nodelistitem_t =
        zmalloc(::std::mem::size_of::<nodelistitem_t>() as libc::c_ulong) as *mut nodelistitem_t;
    let ref mut fresh0 = (*p).curr;
    *fresh0 = n;
    return p;
}
#[no_mangle]
pub unsafe extern "C" fn mkNodelist() -> *mut nodelist_t {
    let mut list: *mut nodelist_t =
        zmalloc(::std::mem::size_of::<nodelist_t>() as libc::c_ulong) as *mut nodelist_t;
    return list;
}
#[no_mangle]
pub unsafe extern "C" fn freeNodelist(mut list: *mut nodelist_t) {
    let mut temp: *mut nodelistitem_t = 0 as *mut nodelistitem_t;
    let mut next: *mut nodelistitem_t = 0 as *mut nodelistitem_t;
    if list.is_null() {
        return;
    }
    temp = (*list).first;
...
#[no_mangle]
pub unsafe extern "C" fn appendNodelist(
    mut list: *mut nodelist_t,
    mut one: *mut nodelistitem_t,
    mut n: *mut Agnode_t,
) {
    let mut np: *mut nodelistitem_t = init_nodelistitem(n);
    let ref mut fresh1 = (*list).sz;
    *fresh1 += 1;
    if one.is_null() {
        one = (*list).last;
    }
    if one == (*list).last {
        if !one.is_null() {
            let ref mut fresh2 = (*one).next;
            *fresh2 = np;
        } else {
            let ref mut fresh3 = (*list).first;
            *fresh3 = np;
        }
        let ref mut fresh4 = (*np).prev;
        *fresh4 = one;
        let ref mut fresh5 = (*np).next;
        *fresh5 = 0 as *mut nodelistitem_t;
        let ref mut fresh6 = (*list).last;
        *fresh6 = np;
    } else {
        let mut temp: *mut nodelistitem_t = (*one).next;
        let ref mut fresh7 = (*one).next;
        *fresh7 = np;
        let ref mut fresh8 = (*np).prev;
        *fresh8 = one;
        let ref mut fresh9 = (*temp).prev;
        *fresh9 = np;
        let ref mut fresh10 = (*np).next;
        *fresh10 = temp;
    };
}
#[no_mangle]
pub unsafe extern "C" fn reverseNodelist(mut list: *mut nodelist_t) -> *mut nodelist_t {
    let mut temp: *mut nodelistitem_t = 0 as *mut nodelistitem_t;
    let mut p: *mut nodelistitem_t = 0 as *mut nodelistitem_t;
    p = (*list).first;
    while !p.is_null() {
        temp = (*p).next;
        let ref mut fresh11 = (*p).next;
        *fresh11 = (*p).prev;
        let ref mut fresh12 = (*p).prev;
        *fresh12 = temp;
        p = (*p).prev;
    }
    temp = (*list).last;
    let ref mut fresh13 = (*list).last;
    *fresh13 = (*list).first;
    let ref mut fresh14 = (*list).first;
    *fresh14 = temp;
    return list;
}
#[no_mangle]
pub unsafe extern "C" fn realignNodelist(mut list: *mut nodelist_t, mut np: *mut nodelistitem_t) {
    let mut temp: *mut nodelistitem_t = 0 as *mut nodelistitem_t;
    let mut prev: *mut nodelistitem_t = 0 as *mut nodelistitem_t;
    if np == (*list).first {
        return;
    }
    temp = (*list).first;
    prev = (*np).prev;
    let ref mut fresh15 = (*list).first;
    *fresh15 = np;
    let ref mut fresh16 = (*np).prev;
    *fresh16 = 0 as *mut nodelistitem_t;
    let ref mut fresh17 = (*(*list).last).next;
    *fresh17 = temp;
    let ref mut fresh18 = (*temp).prev;
    *fresh18 = (*list).last;
    let ref mut fresh19 = (*list).last;
    *fresh19 = prev;
    let ref mut fresh20 = (*prev).next;
    *fresh20 = 0 as *mut nodelistitem_t;
}
#[no_mangle]
pub unsafe extern "C" fn cloneNodelist(mut list: *mut nodelist_t) -> *mut nodelist_t {
    let mut newlist: *mut nodelist_t = mkNodelist();
    let mut temp: *mut nodelistitem_t = 0 as *mut nodelistitem_t;
    let mut prev: *mut nodelistitem_t = 0 as *mut nodelistitem_t;
    temp = (*list).first;
    while !temp.is_null() {
...
#[no_mangle]
pub unsafe extern "C" fn insertNodelist(
    mut list: *mut nodelist_t,
    mut cn: *mut Agnode_t,
    mut neighbor: *mut Agnode_t,
    mut pos: libc::c_int,
) {
    let mut temp: *mut nodelistitem_t = 0 as *mut nodelistitem_t;
    let mut prev: *mut nodelistitem_t = 0 as *mut nodelistitem_t;
    let mut next: *mut nodelistitem_t = 0 as *mut nodelistitem_t;
    let mut actual: *mut nodelistitem_t = 0 as *mut nodelistitem_t;
    temp = (*list).first;
    while !temp.is_null() {
        if (*temp).curr == cn {
            actual = temp;
            ...
            next = (*actual).next;
            if !prev.is_null() {
                let ref mut fresh21 = (*prev).next;
                *fresh21 = next;
            } else {
                let ref mut fresh22 = (*list).first;
                *fresh22 = next;
            }
            if !next.is_null() {
                let ref mut fresh23 = (*next).prev;
                *fresh23 = prev;
            } else {
                let ref mut fresh24 = (*list).last;
                *fresh24 = prev;
            }
            break;
        } else {
    ...
    if !actual.is_null() {
        ...
        __assert_fail(
            ...
            b"nodelist.c\0" as *const u8 as *const libc::c_char,
            161 as libc::c_int as libc::c_uint,
            (*::std::mem::transmute::<&[u8; 63], &[libc::c_char; 63]>(
                b"void insertNodelist(nodelist_t *, Agnode_t *, Agnode_t *, int)\0",
            ))
            .as_ptr(),
        );
    }
    prev = 0 as *mut nodelistitem_t;
    temp = (*list).first;
    while !temp.is_null() {
        if (*temp).curr == neighbor {
            if pos == 0 as libc::c_int {
                if temp == (*list).first {
                    let ref mut fresh25 = (*list).first;
                    *fresh25 = actual;
                    let ref mut fresh26 = (*actual).next;
                    *fresh26 = temp;
                    let ref mut fresh27 = (*actual).prev;
                    *fresh27 = 0 as *mut nodelistitem_t;
                    let ref mut fresh28 = (*temp).prev;
                    *fresh28 = actual;
                    return;
                }
                let ref mut fresh29 = (*prev).next;
                *fresh29 = actual;
                let ref mut fresh30 = (*actual).prev;
                *fresh30 = prev;
                let ref mut fresh31 = (*actual).next;
                *fresh31 = temp;
                let ref mut fresh32 = (*temp).prev;
                *fresh32 = actual;
                return;
            } else {
                if temp == (*list).last {
                    let ref mut fresh33 = (*list).last;
                    *fresh33 = actual;
                    let ref mut fresh34 = (*actual).next;
                    *fresh34 = 0 as *mut nodelistitem_t;
                    let ref mut fresh35 = (*actual).prev;
                    *fresh35 = temp;
                    let ref mut fresh36 = (*temp).next;
                    *fresh36 = actual;
                    return;
                }
                let ref mut fresh37 = (*actual).prev;
                *fresh37 = temp;
                let ref mut fresh38 = (*actual).next;
                *fresh38 = (*temp).next;
                let ref mut fresh39 = (*(*temp).next).prev;
                *fresh39 = actual;
                let ref mut fresh40 = (*temp).next;
                *fresh40 = actual;
                return;
            }
        } else {
...
#[no_mangle]
pub unsafe extern "C" fn sizeNodelist(mut list: *mut nodelist_t) -> libc::c_int {
    return (*list).sz;
}
#[no_mangle]
pub unsafe extern "C" fn node_position(
    mut list: *mut nodelist_t,
    mut n: *mut Agnode_t,
) -> libc::c_int {
    return (*((*((*(n as *mut Agobj_t)).data as *mut Agnodeinfo_t)).alg as *mut cdata))
        .u
        .f
        .pos;
}
unsafe extern "C" fn concatNodelist(mut l1: *mut nodelist_t, mut l2: *mut nodelist_t) {
    if !((*l2).first).is_null() {
        if !((*l2).first).is_null() {
            let ref mut fresh41 = (*(*l1).last).next;
            *fresh41 = (*l2).first;
            let ref mut fresh42 = (*(*l2).first).prev;
            *fresh42 = (*l1).last;
            let ref mut fresh43 = (*l1).last;
            *fresh43 = (*l2).last;
            (*l1).sz += (*l2).sz;
        } else {
            *l1 = *l2;
...
#[no_mangle]
pub unsafe extern "C" fn reverseAppend(mut l1: *mut nodelist_t, mut l2: *mut nodelist_t) {
    l2 = reverseNodelist(l2);
    concatNodelist(l1, l2);
    free(l2 as *mut libc::c_void);
}
