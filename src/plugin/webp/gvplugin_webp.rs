...
extern "C" {
    static mut gvdevice_webp_types: [gvplugin_installed_t; 0];
    static mut gvloadimage_webp_types: [gvplugin_installed_t; 0];
}
...
static mut apis: [gvplugin_api_t; 3] = unsafe {
    [
        {
            let mut init = gvplugin_api_t {
                api: API_device,
                types: gvdevice_webp_types.as_ptr() as *mut _,
            };
            init
        },
        {
            let mut init = gvplugin_api_t {
                api: API_loadimage,
                types: gvloadimage_webp_types.as_ptr() as *mut _,
            };
            init
        },
        {
...
#[no_mangle]
pub static mut gvplugin_webp_LTX_library: gvplugin_library_t = unsafe {
    {
        let mut init = gvplugin_library_t {
            packagename: b"webp\0" as *const u8 as *const libc::c_char as *mut libc::c_char,
