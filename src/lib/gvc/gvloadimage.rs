...
unsafe extern "C" fn gvloadimage_select(
    mut job: *mut GVJ_t,
    ...
    if !plugin.is_null() {
        ...
        let ref mut fresh0 = (*job).loadimage.engine;
        *fresh0 = (*typeptr).engine as *mut gvloadimage_engine_t;
...
#[no_mangle]
pub unsafe extern "C" fn gvloadimage(
    mut job: *mut GVJ_t,
    ...
    let mut gvli: *mut gvloadimage_engine_t = 0 as *mut gvloadimage_engine_t;
    let mut type_0: [libc::c_char; 128] = [0; 128];
    if !job.is_null() {} else {
        __assert_fail(
            b"job\0" as *const u8 as *const libc::c_char,
            b"gvloadimage.c\0" as *const u8 as *const libc::c_char,
            50 as libc::c_int as libc::c_uint,
            (*::std::mem::transmute::<
                &[u8; 68],
                &[libc::c_char; 68],
            >(b"void gvloadimage(GVJ_t *, usershape_t *, boxf, _Bool, const char *)\0"))
                .as_ptr(),
        );
    }
    if !us.is_null() {} else {
    ...
    strcpy(type_0.as_mut_ptr(), (*us).stringtype);
    strcat(type_0.as_mut_ptr(), b":\0" as *const u8 as *const libc::c_char);
    strcat(type_0.as_mut_ptr(), target);
    if gvloadimage_select(job, type_0.as_mut_ptr()) == 999 as libc::c_int {
        agerr(
            AGWARN,
            b"No loadimage plugin for \"%s\"\n\0" as *const u8 as *const libc::c_char,
            type_0.as_mut_ptr(),
        );
    }
    gvli = (*job).loadimage.engine;
    if !gvli.is_null() && ((*gvli).loadimage).is_some() {
        ((*gvli).loadimage).expect("non-null function pointer")(job, us, b, filled);
    }
}
