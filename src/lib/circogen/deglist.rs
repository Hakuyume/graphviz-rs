...
#[no_mangle]
pub unsafe extern "C" fn mkDeglist() -> *mut deglist_t {
    let mut s: *mut deglist_t = dtopen(&mut nodeDisc, Dtoset);
    return s;
}
#[no_mangle]
pub unsafe extern "C" fn freeDeglist(mut s: *mut deglist_t) {
    dtclose(s);
}
#[no_mangle]
pub unsafe extern "C" fn insertDeglist(mut ns: *mut deglist_t, mut n: *mut Agnode_t) {
    let mut key: degitem = degitem {
        link: Dtlink_t {
            right: 0 as *mut Dtlink_t,
            hl: C2RustUnnamed_1 { _hash: 0 },
        },
        deg: 0,
        np: 0 as *mut Agnode_t,
    };
...
#[no_mangle]
pub unsafe extern "C" fn removeDeglist(mut list: *mut deglist_t, mut n: *mut Agnode_t) {
    let mut key: degitem = degitem {
        link: Dtlink_t {
            right: 0 as *mut Dtlink_t,
            hl: C2RustUnnamed_1 { _hash: 0 },
    ...
    if (*ip).np == n {
        ...
        *fresh3 = (*((*(n as *mut Agobj_t)).data as *mut Agnodeinfo_t)).next;
        ...
        prev = (*ip).np;
        np = (*((*(prev as *mut Agobj_t)).data as *mut Agnodeinfo_t)).next;
        while !np.is_null() && np != n {
            prev = np;
...
#[no_mangle]
pub unsafe extern "C" fn firstDeglist(mut list: *mut deglist_t) -> *mut Agnode_t {
    let mut ip: *mut degitem = 0 as *mut degitem;
    let mut np: *mut Agnode_t = 0 as *mut Agnode_t;
    ip = (Some(((*(list as *mut Dt_t)).searchf).expect("non-null function pointer")))
        .expect(
