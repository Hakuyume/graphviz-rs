...
extern "C" {
    ...
    fn makeObstacle(n: *mut node_t, _: *mut expand_t, _: bool) -> *mut Ppoly_t;
    fn makeSelfArcs(e: *mut edge_t, stepx: libc::c_int);
    fn makeSpline(_: *mut edge_t, _: *mut *mut Ppoly_t, _: libc::c_int, _: bool);
...
unsafe extern "C" fn addObj(mut l: *mut objlist, mut obj: *mut Ppoly_t) {
    if (*l).sz == (*l).cnt {
        if !((*l).obs).is_null() {
            (*l).sz *= 2 as libc::c_int;
            let ref mut fresh0 = (*l).obs;
            *fresh0 = grealloc(
                (*l).obs as *mut libc::c_void,
                ((*l).sz as libc::c_ulong)
                    .wrapping_mul(::std::mem::size_of::<*mut Ppoly_t>() as libc::c_ulong),
            ) as *mut *mut Ppoly_t;
        } else {
            let ref mut fresh1 = (*l).obs;
            *fresh1 = gcalloc(
                100 as libc::c_int as size_t,
                ::std::mem::size_of::<*mut Ppoly_t>() as libc::c_ulong,
            ) as *mut *mut Ppoly_t;
            (*l).sz = 100 as libc::c_int;
        }
    }
    let ref mut fresh2 = (*l).cnt;
    let fresh3 = *fresh2;
    *fresh2 = *fresh2 + 1;
    let ref mut fresh4 = *((*l).obs).offset(fresh3 as isize);
    *fresh4 = obj;
}
unsafe extern "C" fn freeObjlist(mut l: *mut objlist) {
    if !l.is_null() {
        free((*l).obs as *mut libc::c_void);
        free(l as *mut libc::c_void);
    }
}
unsafe extern "C" fn resetObjlist(mut l: *mut objlist) {
    (*l).cnt = 0 as libc::c_int;
}
unsafe extern "C" fn makeClustObs(mut g: *mut graph_t, mut pm: *mut expand_t) -> *mut Ppoly_t {
    let mut obs: *mut Ppoly_t =
        zmalloc(::std::mem::size_of::<Ppoly_t>() as libc::c_ulong) as *mut Ppoly_t;
    let mut bb: boxf = boxf {
        LL: pointf { x: 0., y: 0. },
        UR: pointf { x: 0., y: 0. },
    };
    let mut newbb: boxf = boxf {
        LL: pointf { x: 0., y: 0. },
        UR: pointf { x: 0., y: 0. },
    };
    let mut ctr: Ppoint_t = pointf { x: 0., y: 0. };
    bb = (*((*(g as *mut Agobj_t)).data as *mut Agraphinfo_t)).bb;
    (*obs).pn = 4 as libc::c_int;
    let ref mut fresh5 = (*obs).ps;
    *fresh5 = gcalloc(
        4 as libc::c_int as size_t,
        ::std::mem::size_of::<Ppoint_t>() as libc::c_ulong,
    ) as *mut Ppoint_t;
    ctr.x = (bb.UR.x + bb.LL.x) / 2.0f64;
    ctr.y = (bb.UR.y + bb.LL.y) / 2.0f64;
    if (*pm).doAdd {
        newbb.UR.x = bb.UR.x + (*pm).x as libc::c_double;
        newbb.UR.y = bb.UR.y + (*pm).y as libc::c_double;
        newbb.LL.x = bb.LL.x - (*pm).x as libc::c_double;
        newbb.LL.y = bb.LL.y - (*pm).y as libc::c_double;
    } else {
        let mut deltax: libc::c_double = (*pm).x as libc::c_double - 1.0f64;
        let mut deltay: libc::c_double = (*pm).y as libc::c_double - 1.0f64;
        newbb.UR.x = (*pm).x as libc::c_double * bb.UR.x - deltax * ctr.x;
        newbb.UR.y = (*pm).y as libc::c_double * bb.UR.y - deltay * ctr.y;
        newbb.LL.x = (*pm).x as libc::c_double * bb.LL.x - deltax * ctr.x;
        newbb.LL.y = (*pm).y as libc::c_double * bb.LL.y - deltay * ctr.y;
    }
    (*((*obs).ps).offset(0 as libc::c_int as isize)).x = newbb.LL.x;
    (*((*obs).ps).offset(0 as libc::c_int as isize)).y = newbb.LL.y;
    (*((*obs).ps).offset(1 as libc::c_int as isize)).x = newbb.LL.x;
    (*((*obs).ps).offset(1 as libc::c_int as isize)).y = newbb.UR.y;
    (*((*obs).ps).offset(2 as libc::c_int as isize)).x = newbb.UR.x;
    (*((*obs).ps).offset(2 as libc::c_int as isize)).y = newbb.UR.y;
    (*((*obs).ps).offset(3 as libc::c_int as isize)).x = newbb.UR.x;
    (*((*obs).ps).offset(3 as libc::c_int as isize)).y = newbb.LL.y;
    return obs;
}
unsafe extern "C" fn addGraphObjs(
    mut l: *mut objlist,
    mut g: *mut graph_t,
    mut tex: *mut libc::c_void,
    mut hex: *mut libc::c_void,
    mut pm: *mut expand_t,
) {
    ...
    while !n.is_null() {
        if (*((*(n as *mut Agobj_t)).data as *mut Agnodeinfo_t)).clust == g
            && n != tex as *mut node_t
            && n != hex as *mut node_t
            && !(*((*(n as *mut Agobj_t)).data as *mut Agnodeinfo_t)).clustnode
        {
            addObj(l, makeObstacle(n, pm, 0 as libc::c_int != 0));
        }
        n = agnxtnode(g, n);
    }
    i = 1 as libc::c_int;
    while i <= (*((*(g as *mut Agobj_t)).data as *mut Agraphinfo_t)).n_cluster {
        sg = *((*((*(g as *mut Agobj_t)).data as *mut Agraphinfo_t)).clust).offset(i as isize);
        if sg != tex as *mut graph_t && sg != hex as *mut graph_t {
            addObj(l, makeClustObs(sg, pm));
        }
        i += 1;
    }
}
unsafe extern "C" fn raiseLevel(
    mut l: *mut objlist,
    mut maxlvl: libc::c_int,
    mut ex: *mut libc::c_void,
    mut minlvl: libc::c_int,
    mut gp: *mut *mut graph_t,
    mut pm: *mut expand_t,
) {
    let mut g: *mut graph_t = *gp;
    let mut i: libc::c_int = 0;
    i = maxlvl;
    while i > minlvl {
        addGraphObjs(l, g, ex, 0 as *mut libc::c_void, pm);
        ex = g as *mut libc::c_void;
        g = (*((*((*(g as *mut Agobj_t)).data as *mut Agraphinfo_t)).alg as *mut gdata)).parent;
        i -= 1;
    }
    *gp = ex as *mut graph_t;
}
unsafe extern "C" fn objectList(mut ep: *mut edge_t, mut pm: *mut expand_t) -> *mut objlist {
    ...
    let mut t: *mut node_t =
        ...
        .node;
    let mut hg: *mut graph_t = (*((*(h as *mut Agobj_t)).data as *mut Agnodeinfo_t)).clust;
    let mut tg: *mut graph_t = (*((*(t as *mut Agobj_t)).data as *mut Agnodeinfo_t)).clust;
    let mut hlevel: libc::c_int = 0;
    let mut tlevel: libc::c_int = 0;
    let mut hex: *mut libc::c_void = 0 as *mut libc::c_void;
    let mut tex: *mut libc::c_void = 0 as *mut libc::c_void;
    let mut list: *mut objlist =
        zmalloc(::std::mem::size_of::<objlist>() as libc::c_ulong) as *mut objlist;
    if (*((*(h as *mut Agobj_t)).data as *mut Agnodeinfo_t)).clustnode {
        hex = hg as *mut libc::c_void;
        hg = (*((*((*(hg as *mut Agobj_t)).data as *mut Agraphinfo_t)).alg as *mut gdata)).parent;
    } else {
        hex = h as *mut libc::c_void;
    }
    if (*((*(t as *mut Agobj_t)).data as *mut Agnodeinfo_t)).clustnode {
        tex = tg as *mut libc::c_void;
        tg = (*((*((*(tg as *mut Agobj_t)).data as *mut Agraphinfo_t)).alg as *mut gdata)).parent;
    } else {
        tex = t as *mut libc::c_void;
    }
    hlevel = (*((*((*(hg as *mut Agobj_t)).data as *mut Agraphinfo_t)).alg as *mut gdata)).level;
    tlevel = (*((*((*(tg as *mut Agobj_t)).data as *mut Agraphinfo_t)).alg as *mut gdata)).level;
    if hlevel > tlevel {
        raiseLevel(list, hlevel, hex, tlevel, &mut hg, pm);
        hex = hg as *mut libc::c_void;
        hg = (*((*((*(hg as *mut Agobj_t)).data as *mut Agraphinfo_t)).alg as *mut gdata)).parent;
    } else if tlevel > hlevel {
        raiseLevel(list, tlevel, tex, hlevel, &mut tg, pm);
        tex = tg as *mut libc::c_void;
        tg = (*((*((*(tg as *mut Agobj_t)).data as *mut Agraphinfo_t)).alg as *mut gdata)).parent;
    }
    while hg != tg {
        addGraphObjs(list, hg, 0 as *mut libc::c_void, hex, pm);
        addGraphObjs(list, tg, tex, 0 as *mut libc::c_void, pm);
        hex = hg as *mut libc::c_void;
        hg = (*((*((*(hg as *mut Agobj_t)).data as *mut Agraphinfo_t)).alg as *mut gdata)).parent;
        tex = tg as *mut libc::c_void;
        tg = (*((*((*(tg as *mut Agobj_t)).data as *mut Agraphinfo_t)).alg as *mut gdata)).parent;
...
#[no_mangle]
pub unsafe extern "C" fn compoundEdges(
    mut g: *mut graph_t,
    mut pm: *mut expand_t,
    ...
    let mut objl: *mut objlist = 0 as *mut objlist;
    ...
    while !n.is_null() {
        ...
        while !e.is_null() {
            ...
            .node;
            if n == head
                && (*((*(e as *mut Agobj_t)).data as *mut Agedgeinfo_t)).count as libc::c_int != 0
            {
                if P.is_null() {
                    P = zmalloc(::std::mem::size_of::<path>() as libc::c_ulong) as *mut path;
                    let ref mut fresh6 = (*P).boxes;
                    *fresh6 = gcalloc(
            ...
            } else if (*((*(e as *mut Agobj_t)).data as *mut Agedgeinfo_t)).count != 0 {
                objl = objectList(e, pm);
                if Plegal_arrangement((*objl).obs, (*objl).cnt) != 0 {
                    vconfig = Pobsopen((*objl).obs, (*objl).cnt);
                    if vconfig.is_null() {
                        agerr(
                            AGWARN,
                            b"compoundEdges: could not construct obstacles - falling back to straight line edges\n\0"
                        ...
                        e0 = e;
                        while !e0.is_null() {
                            (*((*(e0 as *mut Agobj_t)).data as *mut Agedgeinfo_t)).path =
                                getPath(e0, vconfig, 0 as libc::c_int, (*objl).obs, (*objl).cnt);
                            makeSpline(e0, (*objl).obs, (*objl).cnt, 0 as libc::c_int != 0);
                            e0 = (*((*(e0 as *mut Agobj_t)).data as *mut Agedgeinfo_t)).to_virt;
                        }
                        resetObjlist(objl);
                    }
                } else if rv == 0 as libc::c_int {
                    let mut margin: expand_t = sepFactor(g);
                    let mut pack: libc::c_int = getPack(g, 8 as libc::c_int, 8 as libc::c_int);
                    agerr(
                        AGWARN,
                        b"compoundEdges: nodes touch - falling back to straight line edges\n\0"
                            as *const u8 as *const libc::c_char,
                    );
                    if pack as libc::c_float <= (*pm).x || pack as libc::c_float <= (*pm).y {
                        agerr(
                            AGPREV,
                            b"pack value %d is smaller than esep (%.03f,%.03f)\n\0" as *const u8
                                as *const libc::c_char,
                            pack,
                            (*pm).x as libc::c_double,
                            (*pm).y as libc::c_double,
                        );
                    } else if margin.x <= (*pm).x || margin.y <= (*pm).y {
                        agerr(
                            AGPREV,
                            b"sep value (%.03f,%.03f) is smaller than esep (%.03f,%.03f)\n\0"
                                as *const u8 as *const libc::c_char,
