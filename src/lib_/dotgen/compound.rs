...
unsafe extern "C" fn pf2s(
    mut p: pointf,
    mut buf: *mut libc::c_char,
) -> *mut libc::c_char {
    sprintf(buf, b"(%.5g,%.5g)\0" as *const u8 as *const libc::c_char, p.x, p.y);
    return buf;
}
unsafe extern "C" fn boxIntersectf(
    mut pp: pointf,
    mut cp: pointf,
    mut bp: *mut boxf,
) -> pointf {
    ...
    if cp.x < ll.x {
        ...
        ipp
            .y = pp.y
            + ((ipp.x - ppx) * (ppy - cpy) / (ppx - cpx)) as libc::c_int
                as libc::c_double;
    ...
    if cp.y < ll.y {
        ...
        ipp
            .x = pp.x
            + ((ipp.y - ppy) * (ppx - cpx) / (ppy - cpy)) as libc::c_int
                as libc::c_double;
...
unsafe extern "C" fn inBoxf(mut p: pointf, mut bb: *mut boxf) -> libc::c_int {
    return ((*bb).LL.x <= p.x && p.x <= (*bb).UR.x
        && ((*bb).LL.y <= p.y && p.y <= (*bb).UR.y)) as libc::c_int;
}
...
unsafe extern "C" fn findVertical(
    ...
    Bezier(pts, 3 as libc::c_int, 0.5f64, Left.as_mut_ptr(), Right.as_mut_ptr());
    t = findVertical(
        Left.as_mut_ptr(),
        tmin,
        (tmin + tmax) / 2.0f64,
...
unsafe extern "C" fn splineIntersectf(
    mut pts: *mut pointf,
    mut bb: *mut boxf,
) -> libc::c_int {
    ...
    if t >= 0 as libc::c_int as libc::c_double && t < tmin {
        Bezier(origpts.as_mut_ptr(), 3 as libc::c_int, t, pts, 0 as *mut pointf);
        tmin = t;
    }
    t = findVertical(
        pts,
    ...
    if tmin < 2.0f64 { return 1 as libc::c_int } else { return 0 as libc::c_int };
}
unsafe extern "C" fn makeCompoundEdge(
    ...
    if !lh.is_null() {
        ...
        if inBoxf((*((*(head as *mut Agobj_t)).data as *mut Agnodeinfo_t)).coord, bb)
            == 0
        {
            agerr(
                AGWARN,
            ...
            if inBoxf((*((*(tail as *mut Agobj_t)).data as *mut Agnodeinfo_t)).coord, bb)
                != 0
            {
                agerr(
                    AGWARN,
                    ...
                    agget(
                        e as *mut libc::c_void,
                        b"lhead\0" as *const u8 as *const libc::c_char
                            as *mut libc::c_char,
                    ),
                );
            } else {
                ...
                *((*bez).list)
                    .offset(1 as libc::c_int as isize) = mid_pointf(p, (*bez).sp);
                *((*bez).list)
                    .offset(
                        0 as libc::c_int as isize,
                    ) = mid_pointf(
                    *((*bez).list).offset(1 as libc::c_int as isize),
                    (*bez).sp,
                );
                *((*bez).list)
                    .offset(
            ...
            == 0
        {
                ...
                != 0
            {
                agerr(
                    ...
                    agget(
                        e as *mut libc::c_void,
                        b"ltail\0" as *const u8 as *const libc::c_char
                            as *mut libc::c_char,
                    ),
                );
            } else {
                ...
                *((*bez).list)
                    .offset(
                        (starti + 2 as libc::c_int) as isize,
                    ) = mid_pointf(p, (*nbez).ep);
                *((*bez).list)
                    .offset(
                        (starti + 3 as libc::c_int) as isize,
                    ) = mid_pointf(
                ...
                *((*bez).list)
                    .offset(
                        (starti + 1 as libc::c_int) as isize,
                    ) = mid_pointf(
                    *((*bez).list).offset((starti + 2 as libc::c_int) as isize),
                    p,
                );
            ...
            if starti == 0 as libc::c_int {
                ...
                (*nbez)
                    .sp = boxIntersectf(
                    (*bez).sp,
                    *((*bez).list).offset(starti as isize),
                    bb,
                );
            } else {
                starti -= 3 as libc::c_int;
