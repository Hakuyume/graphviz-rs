...
unsafe extern "C" fn refsymbind(
    ...
    r = (Some(((*(strdict as *mut Dt_t)).searchf).expect("non-null function pointer")))
        ...
        )(strdict, &mut key as *mut refstr_t as *mut libc::c_void, 0o4 as libc::c_int)
        as *mut refstr_t;
    return r;
}
unsafe extern "C" fn refstrbind(
    ...
    if !r.is_null() { return (*r).s } else { return 0 as *mut libc::c_char };
}
#[no_mangle]
...
unsafe extern "C" fn agstrdup_internal(
    ...
    if !r.is_null() {
        ...
        sz = (::std::mem::size_of::<refstr_t>() as libc::c_ulong)
        ...
        *fresh0 = ((*r).store).as_mut_ptr();
        (Some(((*(strdict as *mut Dt_t)).searchf).expect("non-null function pointer")))
            .expect(
                "non-null function pointer",
            )(strdict, r as *mut libc::c_void, 0o1 as libc::c_int);
    }
    return (*r).s;
}
#[no_mangle]
