...
extern "C" {
    static mut gvdevice_types_xlib: [gvplugin_installed_t; 0];
}
    ...
    [
        {
            let mut init = gvplugin_api_t {
                api: API_device,
                types: gvdevice_types_xlib.as_ptr() as *mut _,
            };
            init
        },
        {
...
#[no_mangle]
pub static mut gvplugin_xlib_LTX_library: gvplugin_library_t = unsafe {
    {
        let mut init = gvplugin_library_t {
            packagename: b"xlib\0" as *const u8 as *const libc::c_char as *mut libc::c_char,
