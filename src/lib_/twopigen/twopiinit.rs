...
extern "C" {
    ...
    fn circleLayout(sg: *mut Agraph_t, center: *mut Agnode_t) -> *mut Agnode_t;
...
unsafe extern "C" fn twopi_init_edge(mut e: *mut edge_t) {
    agbindrec(
...
#[no_mangle]
pub unsafe extern "C" fn twopi_init_graph(mut g: *mut graph_t) {
...
unsafe extern "C" fn findRootNode(
    mut sg: *mut Agraph_t,
    mut rootattr: *mut Agsym_t,
) -> *mut Agnode_t {
    let mut n: *mut Agnode_t = 0 as *mut Agnode_t;
    n = agfstnode(sg);
    while !n.is_null() {
        if mapbool(agxget(n as *mut libc::c_void, rootattr)) {
            return n;
        }
        n = agnxtnode(sg, n);
    }
...
#[no_mangle]
pub unsafe extern "C" fn twopi_layout(mut g: *mut Agraph_t) {
    let mut ctr: *mut Agnode_t = 0 as *mut Agnode_t;
    let mut s: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut setRoot: libc::c_int = 0 as libc::c_int;
    let mut setLocalRoot: libc::c_int = 0 as libc::c_int;
    let mut sc: pointf = pointf { x: 0., y: 0. };
    let mut r: libc::c_int = 0;
    let mut rootattr: *mut Agsym_t = 0 as *mut Agsym_t;
    ...
    if !s.is_null() {
        if *s != 0 {
            ctr = agnode(g, s, 0 as libc::c_int);
            if ctr.is_null() {
                agerr(
                    AGWARN,
                    b"specified root node \"%s\" was not found.\0" as *const u8
                        as *const libc::c_char,
                    s,
                );
                agerr(
                    AGPREV,
                    b"Using default calculation for root node\n\0" as *const u8
                        as *const libc::c_char,
                );
                setRoot = 1 as libc::c_int;
            }
        } else {
            setRoot = 1 as libc::c_int;
        }
    ...
    if agnnodes(g) != 0 {
        ...
        let mut c: *mut Agnode_t = 0 as *mut Agnode_t;
        let mut n: *mut Agnode_t = 0 as *mut Agnode_t;
        let mut ncc: libc::c_int = 0;
        let mut i: libc::c_int = 0;
        let mut lctr: *mut Agnode_t = 0 as *mut Agnode_t;
        ccs = ccomps(g, &mut ncc, 0 as *mut libc::c_char);
        if ncc == 1 as libc::c_int {
            if !ctr.is_null() {
                lctr = ctr;
            } else if rootattr.is_null() || {
                lctr = findRootNode(g, rootattr);
                lctr.is_null()
            } {
                lctr = 0 as *mut Agnode_t;
            }
            c = circleLayout(g, lctr);
            if setRoot != 0 && ctr.is_null() {
                ctr = c;
            }
            if setLocalRoot != 0 && lctr.is_null() {
                agxset(
            ...
            *fresh5 = 0 as *mut libc::c_void;
            ...
            while i < ncc {
                ...
                if !ctr.is_null() && agcontains(sg, ctr as *mut libc::c_void) != 0 {
                    lctr = ctr;
                } else if rootattr.is_null() || {
                    lctr = findRootNode(sg, rootattr);
                    lctr.is_null()
                } {
                    lctr = 0 as *mut Agnode_t;
                }
                ...
                if setLocalRoot != 0 && (lctr.is_null() || lctr == ctr) {
                    agxset(
                        c as *mut libc::c_void,
                        rootattr,
                        b"1\0" as *const u8 as *const libc::c_char,
    ...
    if setRoot != 0 {
        agset(
            g as *mut libc::c_void,
            b"root\0" as *const u8 as *const libc::c_char as *mut libc::c_char,
            agnameof(ctr as *mut libc::c_void),
        );
    }
    dotneato_postprocess(g);
}
unsafe extern "C" fn twopi_cleanup_graph(mut g: *mut graph_t) {
...
#[no_mangle]
pub unsafe extern "C" fn twopi_cleanup(mut g: *mut graph_t) {
    let mut n: *mut node_t = 0 as *mut node_t;
    let mut e: *mut edge_t = 0 as *mut edge_t;
    n = agfstnode(g);
    if n.is_null() {
