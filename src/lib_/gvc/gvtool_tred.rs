...
unsafe extern "C" fn dfs(
    ...
    (*((*n).base.data as *mut Agmarknodeinfo_t)).mark = 0 as libc::c_int;
    return warn;
}
#[no_mangle]
pub unsafe extern "C" fn gvToolTred(mut g: *mut Agraph_t) -> libc::c_int {
    ...
    if agisdirected(g) != 0 {
        aginit(
            ...
            ::std::mem::size_of::<Agmarknodeinfo_t>() as libc::c_ulong as libc::c_int,
            (0 as libc::c_int == 0) as libc::c_int,
        );
        n = agfstnode(g);
        while !n.is_null() {
            warn = dfs(n, 0 as *mut Agedge_t, warn);
            n = agnxtnode(g, n);
        }
        agclean(
            g,
        ...
        fprintf(
            stderr,
            b"warning: %s is not a directed graph, not attempting tred\n\0" as *const u8
                as *const libc::c_char,
            agnameof(g as *mut libc::c_void),
        );
    }
