...
extern "C" {
    static mut gvloadimage_rsvg_types: [gvplugin_installed_t; 0];
}
    ...
    [
        {
            let mut init = gvplugin_api_t {
                api: API_loadimage,
                types: gvloadimage_rsvg_types.as_ptr() as *mut _,
            };
            init
        },
        {
...
#[no_mangle]
pub static mut gvplugin_rsvg_LTX_library: gvplugin_library_t = unsafe {
    {
        let mut init = gvplugin_library_t {
            packagename: b"rsvg\0" as *const u8 as *const libc::c_char as *mut libc::c_char,
