...
extern "C" {
    static mut gvdevice_gdk_types: [gvplugin_installed_t; 0];
    static mut gvloadimage_gdk_types: [gvplugin_installed_t; 0];
}
    ...
    [
        {
            let mut init = gvplugin_api_t {
                api: API_device,
                types: gvdevice_gdk_types.as_ptr() as *mut _,
            };
            init
        },
        {
            let mut init = gvplugin_api_t {
                api: API_loadimage,
                types: gvloadimage_gdk_types.as_ptr() as *mut _,
            };
            init
        },
        {
...
#[no_mangle]
pub static mut gvplugin_gdk_LTX_library: gvplugin_library_t = unsafe {
    {
        let mut init = gvplugin_library_t {
            packagename: b"gdk\0" as *const u8 as *const libc::c_char as *mut libc::c_char,
