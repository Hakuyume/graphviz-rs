...
extern "C" {
    static mut gvdevice_types_gtk: [gvplugin_installed_t; 0];
}
    ...
    [
        {
            let mut init = gvplugin_api_t {
                api: API_device,
                types: gvdevice_types_gtk.as_ptr() as *mut _,
            };
            init
        },
        {
...
#[no_mangle]
pub static mut gvplugin_gtk_LTX_library: gvplugin_library_t = unsafe {
    {
        let mut init = gvplugin_library_t {
            packagename: b"gtk\0" as *const u8 as *const libc::c_char as *mut libc::c_char,
