...
unsafe extern "C" fn getenv_path() -> *const libc::c_char {
    let mut path: *const libc::c_char = getenv(b"PATH\0" as *const u8 as *const libc::c_char);
    if !path.is_null() {
        return path;
    }
...
#[no_mangle]
pub static mut opt_info_argv: *mut *mut libc::c_char =
...
pub unsafe extern "C" fn pathpath(
    ...
    return if !s.is_null() && path == buf.as_mut_ptr() {
        strdup(s)
    } else {
        s
    };
