...
#[no_mangle]
pub unsafe extern "C" fn excontext(
    mut p: *mut Expr_t,
    ...
    s = buf;
    if (*p).linep > ((*p).line).as_mut_ptr() || (*p).linewrap != 0 {
        ...
        let ref mut fresh4 = (*p).linep;
        *fresh4 = ((*p).line).as_mut_ptr();
        t = b"<<< \0" as *const u8 as *const libc::c_char as *mut libc::c_char;
        loop {
            let fresh5 = t;
            t = t.offset(1);
            *s = *fresh5;
