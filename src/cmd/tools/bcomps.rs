...
#[no_mangle]
pub static mut outfile: *mut libc::c_char = 0 as *const libc::c_char as *mut libc::c_char;
#[no_mangle]
...
unsafe extern "C" fn blockName(
    ...
    let mut sz: size_t = (strlen(gname)).wrapping_add(128 as libc::c_int as libc::c_ulong);
    if sz > bufsz {
    ...
    if *gname as libc::c_int == '%' as i32 {
        sprintf(
            buf,
            b"_%s_bcc_%d\0" as *const u8 as *const libc::c_char,
            gname,
            d,
        );
    } else {
        sprintf(
            buf,
            b"%s_bcc_%d\0" as *const u8 as *const libc::c_char,
            gname,
            d,
        );
    }
...
unsafe extern "C" fn getName(mut ng: libc::c_int, mut nb: libc::c_int) -> *mut libc::c_char {
    ...
    if ng == 0 as libc::c_int && nb == 0 as libc::c_int {
        ...
        if buf.is_null() {
            let mut sz: size_t =
                (strlen(outfile)).wrapping_add(100 as libc::c_int as libc::c_ulong);
            buf = gv_alloc(sz) as *mut libc::c_char;
        }
        if !suffix.is_null() {
            ...
            sprintf(
                buf,
                b"%s_%d_T\0" as *const u8 as *const libc::c_char,
                path,
                ng,
            );
        } else {
...
unsafe extern "C" fn gwrite(mut g: *mut Agraph_t, mut ng: libc::c_int, mut nb: libc::c_int) {
...
unsafe extern "C" fn mkBlock(mut g: *mut Agraph_t, mut stp: *mut bcstate) -> *mut Agraph_t {
        ...
        if !(v == u) {
            if (*((*v).base.data as *mut Agnodeinfo_t)).val == 0 as libc::c_int {
                ...
                (*((*u).base.data as *mut Agnodeinfo_t)).low =
                    if (*((*u).base.data as *mut Agnodeinfo_t)).low
...
unsafe extern "C" fn process(mut g: *mut Agraph_t, mut gcnt: libc::c_int) -> libc::c_int {
    let mut n: *mut Agnode_t = 0 as *mut Agnode_t;
