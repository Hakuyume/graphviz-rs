...
extern "C" {
    ...
    fn exfreenode(_: *mut Expr_t, _: *mut Exnode_t);
}
        ...
        if sym.is_null() {
            sym = (if !name.is_null() {
                (Some(((*(*ex).symbols).searchf).expect("non-null function pointer")))
                    ...
                    )((*ex).symbols, name as *mut libc::c_void, 0o1000 as libc::c_int)
            } else {
                &mut (*ex).main as *mut Exid_t as *mut libc::c_void
            }) as *mut Exid_t;
        }
        if !sym.is_null() && (*sym).lex == 293 as libc::c_int as libc::c_long
            && !((*sym).value).is_null()
        {
            if type_0 != 258 as libc::c_int {
                return excast(
                    ex,
                    (*(*sym).value).data.procedure.body,
                    type_0,
                    0 as *mut Exnode_t,
                    0 as libc::c_int,
                );
            ...
            (*sym).lex = 287 as libc::c_int as libc::c_long;
            let ref mut fresh0 = (*sym).value;
            *fresh0 = 0 as *mut Exnode_t;
        }
    }
